...
let db = new sqlite3.Database("./finalProject.db", (fehler) => {
  if (fehler) console.log(fehler.message);
  else console.log("Verbindung zum Datenbak hergestellt");
});
...
app.get("/userLastName/i:", (req, res) => {
  db.all(
    `SELECT * FROM 
            users
            WHERE
...
//************************************************
app.get("/products/abruf/alle", (req, res) => {
  db.all(`SELECT * FROM products`, (fehler, zeilen) => {
    if (fehler) {
      res.send(fehler);
    } else {
      res.send(zeilen);
...
app.get(
  "/products/neu/:name/:category/:size/:price/:image/:description/:inhalt",
  (req, res) => {
    db.run(
      `INSERT INTO products
        (name,category,size,price,image,description,inhalt)
        VALUES
        (
        '${req.params.name}',
        '${req.params.category}',
...
app.get("/rezept/abruf/alle", (req, res) => {
  db.all(`SELECT * FROM rezept`, (fehler, zeilen) => {
    if (fehler) {
      res.send(fehler);
    } else {
      res.send(zeilen);
...
///////////////////////////////
app.get("/rezept/abruf/wer/:id", (req, res) => {
  db.all(
    `SELECT * FROM rezept
...
app.get(
  "/products/update/:id/:name/:category/:size/:price/:image/:description",
  (req, res) => {
    db.run(
      `UPDATE products SET
...
app.get("/products/abruf/wer/:id", (req, res) => {
  db.all(
...
app.get("/products/abruf/:id", (req, res) => {
  db.all(
    `SELECT * FROM products
            WHERE productNumber='${req.params.id}'
            `,
...
app.get("/products/abruf/category/:category", (req, res) => {
  db.all(
    `SELECT * FROM products
...
app.get("/user/abruf/alle", (req, res) => {
  db.all(`SELECT * FROM users`, (fehler, zeilen) => {
    if (fehler) {
      res.send(fehler);
    } else {
      res.send(zeilen);
...
app.get(
  "/user/neu/:userNickName/:userFirstName/:userLastName/:userType/:eMail/:phone/:adresLine1/:adresLine2/:city/:state/:postalCode/:country/:password",
  (req, res) => {
    db.run(
      `INSERT INTO users
...
app.get(
  "/user/update/:userNumber/:userNickName/:userLastName/:userFirstName/:userType/:eMail/:phone/:adresLine1/:adresLine2/:city/:state/:postalCode/:country/:password",
  (req, res) => {
    db.run(
      `UPDATE users SET
...
app.get("/user/delete/:userNumber", (req, res) => {
  db.run(
    `DELETE FROM users
...
app.get("/user/abruf/wer/:userNumber", (req, res) => {
  db.all(
    `SELECT * FROM users
                    WHERE userNumber='${req.params.userNumber}'
                    `,
...
/**************Order***********************/
app.get("/orders/abruf/alle", (req, res) => {
  db.all(`SELECT * FROM orders`, (fehler, zeilen) => {
    if (fehler) {
      res.send(fehler);
    } else {
      res.send(zeilen);
...
app.get("/order/neu/:userNumber/:orderDate/:totalAmount", (req, res) => {
  db.run(
    `INSERT INTO orders
...
app.get("/order/delete/:orderNumber", (req, res) => {
  db.run(
    `DELETE FROM orders
...
/*************OrderDetail********************/
app.get("/orderdetail/abruf/alle", (req, res) => {
  db.all(`SELECT * FROM orderDetails`, (fehler, zeilen) => {
    if (fehler) {
      res.send(fehler);
    } else {
      res.send(zeilen);
...
app.get(
  "/orderdetail/neu/:orderNumber/:productNumber/:quantity/:price",
  (req, res) => {
    db.run(
...
app.get(
  "/favorites/update/:userNumber/:productNumber/:favorite",
  (req, res) => {
    db.run(
...
/********************Nutrition**************************/
app.get("/nutrition/abruf/:productNumber", (req, res) => {
  db.all(
...
const server = app.listen(Portnummer, () => {
  console.log(
    `Der Backend-Server mit der Adresse http://localhost:${Portnummer}/ ist aktiv!`
  );
});
